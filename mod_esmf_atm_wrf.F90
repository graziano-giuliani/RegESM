...
!-----------------------------------------------------------------------
      ...
      if (head_grid%bdy_mask(2)) then
        ptrX(ubnd(1),:) = ptrX(ubnd(1)-1,:) + &
            (ptrX(ubnd(1)-1,:)-ptrX(ubnd(1)-2,:))
        ptrY(ubnd(1),:) = ptrY(ubnd(1)-1,:) + &
            (ptrY(ubnd(1)-1,:)-ptrY(ubnd(1)-2,:))
      end if

      if (head_grid%bdy_mask(4)) then
         ptrX(:,ubnd(2)) = ptrX(:,ubnd(2)-1) + &
             (ptrX(:,ubnd(2)-1)-ptrX(:,ubnd(2)-2))
         ptrY(:,ubnd(2)) = ptrY(:,ubnd(2)-1) + &
             (ptrY(:,ubnd(2)-1)-ptrY(:,ubnd(2)-2))
      end if
...
!-----------------------------------------------------------------------
      ...
      if (head_grid%bdy_mask(2)) then
        do k = 1, kz
        ptrX(ubnd(1),:,k) = ptrX(ubnd(1)-1,:,k) + &
            (ptrX(ubnd(1)-1,:,k)-ptrX(ubnd(1)-2,:,k))
        ptrY(ubnd(1),:,k) = ptrY(ubnd(1)-1,:,k) + &
            (ptrY(ubnd(1)-1,:,k)-ptrY(ubnd(1)-2,:,k))
      ...
      if (head_grid%bdy_mask(4)) then
        do k = 1, kz
        ptrX(:,ubnd(2),k) = ptrX(:,ubnd(2)-1,k) + &
            (ptrX(:,ubnd(2)-1,k)-ptrX(:,ubnd(2)-2,k))
        ptrY(:,ubnd(2),k) = ptrY(:,ubnd(2)-1,k) + &
            (ptrY(:,ubnd(2)-1,k)-ptrY(:,ubnd(2)-2,k))
...
!-----------------------------------------------------------------------
      ...
      case ('taux')
        ...
        do m = ips, ipe
          do n = jps, jpe
            ...
            cff3 = 0.5*(head_grid%u_2(m,1,n) + &
                head_grid%u_2(m+1,1,n))*head_grid%cosa(m,n)-&
                   0.5*(head_grid%v_2(m,1,n) + &
                   head_grid%v_2(m,1,n+1))*head_grid%sina(m,n)
      ...
      case ('tauy')
        ...
        do m = ips, ipe
          do n = jps, jpe
            ...
            cff2 = 2.0/(((head_grid%u_2(m,1,n)+head_grid%u_2(m+1,1,n))**2+&
                   (head_grid%v_2(m,1,n)+head_grid%v_2(m,1,n+1))**2)**0.5+eps)
            cff3 = 0.5*(head_grid%v_2(m,1,n) + &
                head_grid%v_2(m,1,n+1))*head_grid%cosa(m,n)+&
                   0.5*(head_grid%u_2(m,1,n) + &
                   head_grid%u_2(m+1,1,n))*head_grid%sina(m,n)
            cff = cff1*cff2*(head_grid%UST(m,n)**2)*cff3
            ptr2d(m,n) = cff
          end do
        end do
      case ('wspd') ! m/s
        do m = ips, ipe
          do n = jps, jpe
            cff1 = 0.5*(head_grid%U10(m,n) + &
                head_grid%U10(m+1,n))*head_grid%cosa(m,n)-&
                   0.5*(head_grid%V10(m,n) + &
                   head_grid%V10(m,n+1))*head_grid%sina(m,n)
            cff2 = 0.5*(head_grid%V10(m,n) + &
                head_grid%V10(m,n+1))*head_grid%cosa(m,n)+&
                   0.5*(head_grid%U10(m,n) + &
                   head_grid%U10(m+1,n))*head_grid%sina(m,n)
...
!-----------------------------------------------------------------------
 ...
 100  format(A10,'_',A,'_',                                             &
    ...
    subroutine wrf_vintrp(ips, ipe, jps, jpe, kps, kpe, bdy_mask, &
            hgt, varin, varout)
      implicit none
